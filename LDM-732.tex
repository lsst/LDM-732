...
The Data Management Subsystem Requirements (\citeds{LSE-61}, aka DMSR) drive the LSST 
Observatory Network Design for all segments except the Summit Network (see above).  
The DMSR sections that directly drive the LONVP are listed here for convenience. 
These DMSR sections contain lead to traceable derived network requirements as documented 
in the LONVM (and they are listed in the LONVM in their original form).
...
In addition, while some of the DM requirements do not explicitly identify nor constrain the Networks, 
they do specify latency requirements and as such lead to traceable derived LONVP requirements as 
documented in the LONVM.  For each parent requirement in these sections, a derived network requirement 
was created with “-NET” appended to the ID of the parent requirement.
